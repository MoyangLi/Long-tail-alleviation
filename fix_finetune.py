...
# pydevd_pycharm.settrace('222.20.96.71', port=22, stdoutToServer=True, stderrToServer=True)
...
"""
    cifar-10 数据集是自动打散的, 所以不需要重新进行打散
"""
parser = argparse.ArgumentParser(description='PyTorch FixMatch Fine-tuning')
...
# Checkpoints
parser.add_argument('--resume', default='./FixMatch_result/model_300.pth.tar', type=str, metavar='PATH',    # required=True,    # required=true 表示前端必须传参数
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--out', default='Fix_finetune_result',
...
# Sampler options
parser.add_argument('--sampler', default='mean', type=str, choices=['random', 'mean', 'reverse'],
                    help='Sampler for labeled data')
parser.add_argument('--semi-sampler', default='mean', type=str, choices=['random', 'mean', 'reverse'],
...
def main():
    ...
    if args.sampler == 'mean':
        labeled_trainloader = data.DataLoader(train_labeled_set, batch_size=args.batch_size, sampler=mean_sampler, num_workers=args.num_workers, drop_last=True)
    elif args.sampler == 'reverse':
        labeled_trainloader = data.DataLoader(train_labeled_set, batch_size=args.batch_size, sampler=reverse_sampler, num_workers=args.num_workers, drop_last=True)
    ...
    # load checkpoint: path to latest checkpoint (default: none)
    ...
    print('Finetune params: %.6fM' % (float(sum(p.numel() for p in model.parameters() if p.requires_grad))/1000000.0))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # 存储 finetune 之后的结果
        np.savez('./Fix_finetune_result', Loss=Loss, Acc=Acc)
